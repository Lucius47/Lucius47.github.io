...
class Project {
	...
	descHtml() {
		if (this.desc == `` || this.desc == undefined) return ``;

		let str = `<div class="proj-desc">${this.desc}</div>`;
		if (this.company != undefined) return `${str}`;
		return str;
	}
	...
	descTimespan() {
		if (this.hasImageCard()) return ``; // no timestamp for image cards
		return `<div class="timespan">${
			this.timespan == undefined ? `` : this.timespan
		}</div>`;
	}
	imgCardHtml() {
	...
	containerClass() {
		if (this.card == undefined) return `proj-container`;
		return `proj-container-vertical`;
	}

	containerTitleClass() {
		if (this.card == undefined) return `proj-title"`;
...
window.onload = function () {
	...
	document.getElementById("social_icons").innerHTML =
		...
		socialIcon(
			"Twitter",
			"https://twitter.com/MuneebShaw",
			`easiest way to follow what I do`
		) +
		socialIcon(
			"LinkedIn",
			"https://www.linkedin.com/in/MuneebShaw",
			`for conformity`
		) +
		socialIcon(
			"Instagram",
			"https://www.instagram.com/Muneeb.Shaw/",
			`food and travel pics`
		) +
		socialIcon(
			"tumblr",
			"https://MuneebShaw.tumblr.com/",
			`mostly art and writing`
		) +
		socialIcon(
			"Mastodon",
			"https://mastodon.social/@Muneeb.Shaw",
			`just in case twitter burns to the ground`
		) +
		socialIcon(
			"Twitch",
			"https://www.twitch.tv/Muneeb.Shaw",
			`game dev, D&D & DDR streams`
		) +
		socialIcon("Discord", "https://discord.gg/v5VWugaX", `my community`) +
		socialIcon(
			"ArtStation",
			"https://www.artstation.com/MuneebShaw",
			`my art & illustrations`
		) +
		socialIcon(
			"YouTube",
			"https://www.youtube.com/Muneeb.Shaw",
			`unity tutorials & other videos`
		) +
		socialIcon(
			"Patreon",
			"https://www.patreon.com/Muneeb.Shaw",
	...
	let university = new Project(
		`Bachelor of Science in Mechanical Engineerng`,
		undefined,
		`2017-2021`,
	...
	let uniFyp = new Project(
		`Design, Simulation, and Testing of UAV based packages delivery system`,
		`https://www.researchgate.net/publication/355023499_Design_and_Fabrication_of_a_Quadcopter_Delivery_Drone`,
		`2021`,
		`as a final year project, designed, moddeled, and fabricated a quadcopter. Designed a system for GPS based packages delivery.`,
		[new Company("", "", ""), uetp]
	);
	...
	// document.getElementById("projects-past").innerHTML = uniFyp;
};
